...
stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = lib.optional stdenv.isLinux [ dpkg ];
  ...
  installPhase = ''
    ...
    then
      ...
      # Modernize socket path.
      substituteInPlace "$out/etc/blackfire/agent.dist" \
        --replace '/var/run' '/run'
    fi
  ...
  passthru = {
    updateScript = writeShellScript "update-${pname}" ''
      ...
      export NEW_VERSION=$(curl -s https://blackfire.io/api/v1/releases | jq .cli --raw-output)
